...
def main(exp_name,data_path, is_code_testing=False, is_model_test=False):

    # util.write_pykeen_dataset()
    # sys.exit(0)
    ...
    config["exp_name"] = config["dataset_name"]+"_"+config["neg_sampler"]+"_"+str(config["num_negs"])
    results_dir =  os.path.join(data_path,config["exp_name"])
    ...
    util.dump_json(config, results_dir, 'config.json')
    ...
    data_set = data_loader.read_dataset(
        os.path.join(data_path, config["dataset_name"]),
        results_dir,
        dev_mode=True,
    ...
    train_file = os.path.join(data_path, config["dataset_name"], "train")
    valid_file = os.path.join(data_path, config["dataset_name"], "dev")
    test_file = os.path.join(data_path, config["dataset_name"], "test")
...
# endregion
    ...
    print("Writing trained model to directory: {}".format(results_dir))
    _model.save_state(os.path.join(results_dir,"TransE"))
    util.dump_json(losses_per_epoch, results_dir, "losses_per_epoch.json")
...
# endregion
    ...
    util.dump_json(results_dict,results_dir,'evaluation_results.json')
    ...
    util.dump_json(my_results_dic,results_dir,'my_results.json')
...
if __name__=='__main__':
    ...
    parser.add_argument('--model_test', dest='is_model_testing', action='store_true', help='A boolean flag')
    
    args = parser.parse_args()
    main(args.exp_name,args.data_path, args.is_code_testing, args.is_model_test)
